...
date: "2020-04-03"
tags: ["知乎", "Web", "React"]
origin: "转载自[我在知乎上的回答](https://www.zhihu.com/question/378861485/answer/1125724740)"
---

有人在[知乎上问](https://www.zhihu.com/question/378861485/answer/1125724740)：

> React Hooks 感觉很反直觉——组件函数每次渲染都要跑一遍，`useState` 背后是全局链表黑魔法，`useEffect` 的语义不直观，为了暂存函数还得专门搞 `useCallback`。Vue 3 的 Composition API 借鉴了 Hooks 的一些思路，但感觉更自然：`setup` 只跑一次，return 一个 render 函数，直接利用 JS 闭包来保存组件状态。**React 是否也可以用 Vue 的这种方式来改进自己的 Hooks？**

不太能，因为是很不一样的心智模型（Mental Model）。很多同学只关注到了这两套 API 在功能上都能复用逻辑的相似点，而低估了两个框架体系「大背景」上的差异。让我展开说说。
