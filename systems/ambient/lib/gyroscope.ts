...
/**
 * How long the sensor's own jitter is smoothed over, in seconds.
 *
 * This is a property of the SENSOR, not of a raindrop, which is why it lives
 * here and not in the renderer's easing. An accelerometer at rest on a table
 * still reports a degree or so of wander, and the weather is re-aimed fast
 * enough (a raindrop really does turn within a blink) that passing it through
 * unfiltered would show as a shimmer along the rain. Short enough that a real
 * turn of the wrist is not noticeably delayed by it.
 */
const SENSOR_TAU = 0.12;
...
function onDeviceOrientation(event: DeviceOrientationEvent) {
  ...
  const raw = gravityFromOrientation(event.beta, event.gamma, readScreenAngle());
  const now = performance.now();
  // Measured against the clock rather than counted in events: these arrive at
  // whatever rate the device feels like, and a fixed per-event weight would
  // smooth twice as hard on a 120 Hz sensor as on a 60 Hz one.
  const dt = lastAt === 0 ? Infinity : Math.min((now - lastAt) / 1000, 1);
  lastAt = now;
  const k = 1 - Math.exp(-dt / SENSOR_TAU);
  const x = latest.x + (raw.x - latest.x) * k;
  const y = latest.y + (raw.y - latest.y) * k;
  const len = Math.hypot(x, y);
  latest = len < 1e-4 ? raw : { x: x / len, y: y / len };
