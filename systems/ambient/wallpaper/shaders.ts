...
void main() {
  ...
  float mie = exp(-sunDist * mix(6.0, 3.2, uWarmth)) * uSunGlow;
  mie *= smoothstep(-0.18, 0.05, uSunElevation);
  col += uSunColor * mie * mix(0.55, 1.15, uWarmth);

  float horizonWarm = exp(-abs(uv.y - 0.32) * 6.0) * uWarmth * smoothstep(0.22, 0.0, abs(uSunElevation));
  col = mix(col, uSunColor, horizonWarm * 0.28);

  float sunDisk = smoothstep(0.034 * uSunScale, 0.016 * uSunScale, sunDist);
  sunDisk *= smoothstep(-0.04, 0.08, uSunElevation);
  col += uSunColor * sunDisk * 1.35;
  ...
  float softness = mix(0.08, 0.28, uCloudSoftness);
  float clouds = smoothstep(threshold - softness, threshold + softness * 0.85, field);
  clouds *= mix(0.35, 1.0, smoothstep(0.02, 0.42, uv.y));
  clouds *= mix(1.0, 0.55, uCloudCover * step(0.82, uCloudCover) * (1.0 - uv.y));

  float ddx = fbm(cUv * vec2(2.1, 1.25) + drift + vec2(0.05, 0.0)) - n1;
  float lit = clamp(0.35 + ddx * 4.5 + uSunElevation * 0.35, 0.0, 1.0);
  vec3 cloudCol = mix(uCloudShadow, uCloudLit, lit);
  cloudCol = mix(cloudCol, uCloudShadow, uCloudDarkness * 0.75);
  col = mix(col, cloudCol, clouds * mix(0.55, 0.92, uCloudCover));
