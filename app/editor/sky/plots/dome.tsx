...
export function SkyDomePlot({
  ...
  const horizonEvents = [
    { az: azAt(sunTrack, events.sun.rise), color: SUN_COLOR, label: at(events.sun.rise) },
    { az: azAt(sunTrack, events.sun.set), color: SUN_COLOR, label: at(events.sun.set) },
    { az: azAt(moonTrack, events.moon.rise), color: MOON_COLOR, label: at(events.moon.rise) },
    { az: azAt(moonTrack, events.moon.set), color: MOON_COLOR, label: at(events.moon.set) },
  ...
  return (
    <Plot
      ...
      svgClassName="h-44"
      legend={[
        { label: L.sun, color: SUN_COLOR },
        { label: L.moon, color: MOON_COLOR },
      ]}
    >
      ...
      <GridLine y={HORIZON_Y} x2={DOME_W} label={L.horizon} strong />
      <GridLine y={domeY(-18)} x2={DOME_W} label={L.astronomicalNight} />
      ...
      <Marker
        ...
        color={SUN_COLOR}
        r={4}
        title={`${L.sun} ${sun.elevation.toFixed(1)}° / ${Math.round(sun.azimuth)}°`}
      />
      <Marker
        x={moon.azimuth}
        ...
        color={MOON_COLOR}
        r={4}
        title={`${L.moon} ${moon.elevation.toFixed(1)}° / ${Math.round(moon.azimuth)}°`}
      />
    </Plot>
  );
}
...
export function ScreenPlot({
  ...
  return (
    <Plot
      ...
      legend={[
        { label: L.sun, color: SUN_COLOR },
        { label: L.moonShown, color: STAGE_COLOR },
        { label: L.moonStaged, color: MOON_COLOR, dashed: true },
      ]}
    >
