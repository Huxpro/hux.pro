...
export function AnalemmaPlot({
  ...
  return (
    <Plot
      title={L.analemma}
      hint={L.analemmaHint(clockLabel)}
      viewBox={`0 0 ${A_W} ${A_H}`}
      svgClassName="h-48"
      footer={
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-baseline justify-between">
        ...
        <span className="font-mono text-[10px] tabular-nums text-tertiary-foreground">
          {L.monthPeak(`${maxTransit.toFixed(0)}°`)}
        </span>
      </div>
        ...
        {days.map((d) => {
          ...
          return (
            <button
              ...
              title={L.monthTile(
                date(d.ms, locale),
                Math.round(d.illumination * 100),
                `${d.transitElevation.toFixed(0)}°`
              )}
              className={cn(
...
export function PhaseDial({
  ...
  return (
    <div className="flex items-center gap-3">
      <svg viewBox="0 0 68 68" className="h-16 w-16 shrink-0" role="img" aria-label={L.phaseDial}>
