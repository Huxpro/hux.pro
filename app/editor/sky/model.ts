...
export interface Observer {
  /** Already in the reader's language — the panel resolves it. */
  label: string;
  lat: number;
  lon: number;
}

/** The built-in places, as dictionary keys (see `i18n.ts`). */
 ...
 */
export const OBSERVER_PRESETS: { key: ObserverKey; lat: number; lon: number }[] = [
  { key: "cityLondon", lat: 51.5074, lon: -0.1278 },
  { key: "cityNewYork", lat: 40.7128, lon: -74.006 },
  { key: "cityShanghai", lat: 31.2304, lon: 121.4737 },
  { key: "citySingapore", lat: 1.3521, lon: 103.8198 },
  { key: "citySydney", lat: -33.8688, lon: 151.2093 },
  { key: "cityUshuaia", lat: -54.8019, lon: -68.303 },
  { key: "cityTromso", lat: 69.6492, lon: 18.9553 },
];

/** London, so the lab has a sky before the visitor's location resolves. */
export const DEFAULT_OBSERVER = OBSERVER_PRESETS[0];
...
export interface Scenario {
  id: string;
  /** Dictionary keys for the chip and its tooltip (see `i18n.ts`). */
  labelKey: string;
  whyKey: string;
...
export const SCENARIOS: Scenario[] = [
  {
    id: "full-moon",
    labelKey: "scenarioFullMoon",
    whyKey: "scenarioFullMoonWhy",
    condition: "clear",
    instant: (ms, lat, lon) => moonHigh(findPhaseDay(ms, 0.5), lat, lon),
  },
  {
    id: "new-moon",
    labelKey: "scenarioNewMoon",
    whyKey: "scenarioNewMoonWhy",
    condition: "clear",
    instant: (ms) => startOfLocalDay(findPhaseDay(ms, 0)) + 24 * 3_600_000,
  },
  {
    id: "day-moon",
    labelKey: "scenarioDayMoon",
    whyKey: "scenarioDayMoonWhy",
    condition: "clear",
    instant: (ms, lat, lon) => moonHigh(findPhaseDay(ms, 0.35), lat, lon),
  },
  {
    id: "storm-dusk",
    labelKey: "scenarioStormDusk",
    whyKey: "scenarioStormDuskWhy",
    condition: "thunder",
    instant: (ms, lat, lon) => sunsetish(ms, lat, lon),
  },
  {
    id: "blue-hour",
    labelKey: "scenarioBlueHour",
    whyKey: "scenarioBlueHourWhy",
    condition: "clear",
    instant: (ms, lat, lon) => sunsetish(ms, lat, lon) + 18 * 60_000,
  },
  {
    id: "polar-night",
    labelKey: "scenarioPolarNight",
    whyKey: "scenarioPolarNightWhy",
    condition: "snow",
...
export interface ReferenceCheck {
  /** Dictionary keys for the row and its provenance (see `i18n.ts`). */
  labelKey: string;
  sourceKey: string;
  /** How many lunations the synodic row averaged, for its label. */
  sample?: number;
  published: string;
  measured: string;
  error: string;
...
export function referenceChecks(): ReferenceCheck[] {
  ...
  cached = [
    ...
    {
      labelKey: "refSynodic",
      sourceKey: "refSynodicSource",
      sample: SYNODIC_SAMPLE,
    ...
    {
      labelKey: "refObliquity",
      sourceKey: "refObliquitySource",
      published: "23.4393\u00b0",
...
/**
 * Dates and clocks in the reader's language.
 *
 * A 24-hour clock either way — this is a workbench, and `18:14` beside a
 * published almanac value should read as the same kind of number in both
 * languages. Only the date parts change: `15 Sept 2026` / `2026年9月15日`.
 */
type Formats = {
...
export const clock = (ms: number | null | undefined, locale = "en"): string =>
  typeof ms === "number" && Number.isFinite(ms)
    ? formats(locale).clock.format(ms)
    : "—";
export const date = (ms: number, locale = "en"): string =>
  formats(locale).date.format(ms);
export const stamp = (ms: number, locale = "en"): string =>
  formats(locale).stamp.format(ms);
