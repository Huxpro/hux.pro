...
export const SECTIONS: DocSection[] = [
  {
    ...
    lede: {
      ...
      zh: "窗玻璃（Vitre）让 theme-color 在 iOS 26 上重新可用：实时给 Safari 的玻璃工具栏和状态栏着色，在页面四周画一圈 bezel，并让页面在容器里滚动，边缘保持稳定。左边的手机就是 demo 站点，下面每一节都会驱动它演示。",
    },
    body: {
      en: (
        <>
    ...
    body: {
      en: (
        <>
          ...
          <p><code>&lt;Vitre&gt;</code> keeps its attributes on <code>&lt;html&gt;</code>: when something strips them, as React 19 does after a failed hydration, they are back before the next paint.</p>
      ...
      zh: (
        <>
          ...
          <p><code>&lt;Vitre&gt;</code> 会守住它写在 <code>&lt;html&gt;</code> 上的属性：被清空时（比如 hydration 失败后的 React 19），会在下一次绘制前恢复。</p>
          <Fields fields={BEZEL_PROPS} />
        </>
      ),
    },
...
<Vitre color={tint === "theme" ? ground : tint} ground={ground} … />`,
  ...
  },
  {
    ...
    lede: {
      en: "scroll picks the page's scroller: the window, or a container inside the bezel with <html> and <body> held still. <Vitre> writes the choice to <html>, and everything else reads it from there.",
      zh: "scroll 决定页面的滚动者：window，或者 bezel 里的一个容器，此时 <html> 和 <body> 固定不动。<Vitre> 把这个选择写到 <html> 上，其他地方都从那里读取。",
    },
    body: {
      en: (
        <>
  ...
  },
  {
    ...
    body: {
      ...
      zh: (
        <>
          <p>iOS 把这个手势交给 window 的滚动视图。页面离开顶部时，窗玻璃把 window 往下停几个像素，因为 <code>&lt;body&gt;</code> 是固定的，所以看不出来；Safari 把它滚回 0，就是这次点击。随后容器在 280–640ms 内（按距离）平滑回到顶部。开启减弱动态效果时直接跳到顶部，手指一碰就停在当前位置。</p>
  ...
  },
  {
    ...
    body: {
      en: (
        <>
          <p>So a change is shown to Safari as a morph: a fixed bezel in the new colour grows from the band on screen to <code>CHROME_MORPH_PX</code> (160ms), holds while Safari samples it (440ms), and eases back (280ms). Each band is its own fixed element — a transparent container with coloured children does not work — and they are children of <code>&lt;html&gt;</code>, so container scroll leaves them fixed. <code>theme-color</code> is set too, for iOS 18. <code>&lt;Vitre&gt;</code> calls it whenever the colour the chrome should show changes.</p>
          <Fields fields={CHROME_SYNC_OPTIONS} />
        </>
      ),
      zh: (
        <>
          <p>所以每次变色都用一次“变形”告诉 Safari：一个新颜色的 fixed bezel 从当前 band 长到 <code>CHROME_MORPH_PX</code>（160ms），停留让 Safari 取色（440ms），再缓缓收回（280ms）。每条 band 都是独立的 fixed 元素（透明容器里放彩色子元素是不行的），并且挂在 <code>&lt;html&gt;</code> 下，container 滚动时不会被改成 absolute。同时也会设置 <code>theme-color</code>，照顾 iOS 18。只要 chrome 应该显示的颜色变了，<code>&lt;Vitre&gt;</code> 就会调用它。</p>
          <Fields fields={CHROME_SYNC_OPTIONS} />
        </>
      ),
    },
  ...
  },
  {
    ...
    body: {
      en: (
        <>
          <p>The resolver is the body of a function, as source, that returns a <code>VitreBootState</code>. It runs before React, so it cannot import; interpolate your constants into it. This demo&apos;s resolver reads its saved devtool settings.</p>
          <Fields fields={BOOT_STATE} />
        </>
      ),
      zh: (
        <>
          <p>resolver 是一个函数体的源码，返回一个 <code>VitreBootState</code>。它在 React 之前运行，不能 import，常量需要插值进去。这个 demo 的 resolver 读取的是 devtool 保存的设置。</p>
          <Fields fields={BOOT_STATE} />
        </>
      ),
    },
    ...
    body: {
      en: (
        <>
          ...
          <Code code={REBIND} />
          <h3>The page</h3>
          <p>The helpers are built on the scroller and read the mode at every call. Without <code>&lt;Vitre&gt;</code> they act on the window. <code>scrollPageTo</code> takes the platform&apos;s scroll behaviour:</p>
      ...
      zh: (
        <>
          ...
          <Code code={REBIND} />
          <h3>页面</h3>
          <p>这些工具函数建立在滚动者之上，每次调用都读取当前模式。没有 <code>&lt;Vitre&gt;</code> 时作用于 window。<code>scrollPageTo</code> 接受平台的滚动行为参数：</p>
          <Fields fields={SCROLL_PAGE_OPTIONS} />
        </>
      ),
    },
...
scrollPageTo(pageOffsetOf(heading) - 96, { behavior: "smooth" });`,
  ...
  },
  {
    ...
    lede: {
      en: "useVitre() returns the state the bezel is showing. Outside <Vitre> it returns a disabled default.",
      zh: "useVitre() 返回 bezel 当前显示的状态。在 <Vitre> 之外返回一个关闭状态的默认值。",
    },
