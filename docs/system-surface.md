...
| Theater playlist | `ADAPTIVE_PRESENTATION` | Albums + tracks for the video player. Its detents are measured rather than declared (`playlistDetents`): the sheet stops under whatever shape the player is wearing — the PiP window, which the provider parks at the top of the screen while the list is up, or the dock card it collapsed into — so on a phone the two split the screen instead of overlapping, and the video is never something the list has to work around. The tablet panel can't resize to that, so there it ends above the window instead |
