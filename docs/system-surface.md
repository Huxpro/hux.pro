...
| Theater playlist | `ADAPTIVE_PRESENTATION` | Albums + tracks for the video player; detents `[0.5, 1]` (`THEATER_PLAYLIST_DETENTS`) rather than the site's, because the PiP window keeps playing in the half of the phone screen the sheet leaves — the provider parks it above the sheet's opening detent, and it floats over the sheet (z-10004 against the viewport's z-60) so the video is never hidden |
