...
| Identity card | `ANCHORED_PRESENTATION` | Who signed a commit: a profile card. `fitContent`; the popover hangs off whichever `<handle>` or `Role:` was pressed, the anchor kept in a ref by its provider. See [Identity System](./system-identity.md) |
