...
Fonts are **baked into the repo** (`lib/og-fonts/`, generated by
`scripts/og-fonts.mjs`) and read from disk — **no network at build time**.
Fetching them during `next build` was fragile: ~40 OG routes fire hundreds of
Google Fonts requests, and a single rate-limited/failed fetch throws and fails
the whole build (this is what broke the Vercel deployment). The baked subsets
are tiny (~310 KB total): Latin faces cover the full printable range (any English
title works forever), and the CJK face is subset to current post-title glyphs
plus a common-Hanzi cushion. Cover images are likewise read from disk and inlined
as data URIs — consistent with the project's "bake it at build" philosophy.

**Refresh fonts** after adding a Chinese title with new glyphs:
